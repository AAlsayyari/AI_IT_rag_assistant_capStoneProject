...
# تصحيح مسار الاستيراد بناءً على شجرة المجلدات الخاصة بك (Evaluations.Eval)
...
def format_metric_html(
    ...
    return f"""
    <div style="margin: 12px 0; padding: 15px; background-color: #f5f5f5; border-radius: 8px; border-left: 5px solid {color};">
...
def run_retrieval_evaluation(progress=gr.Progress()):
    ...
    if count == 0:
        return "<div>لا توجد نتائج للتقييم</div>", pd.DataFrame()
        ...
        with gr.Row():
            with gr.Column(scale=1):
                retrieval_metrics = gr.HTML(
                    "<div style='padding: 50px; text-align: center; color: #999;'>Click 'Run Evaluation' to start</div>"
                )
